...
def main():
    ...
    for symbol_ohlcv in symbols_ohlcv:
        ...
        df.rename(columns={f'{symbol}.MI.Open': 'Open',
                           f'{symbol}.MI.High': 'High',
                           f'{symbol}.MI.Low': 'Low',
                           f'{symbol}.MI.Close': 'Close',
                           f'{symbol}.MI.Volume': 'Volume'}, inplace=True)
        df.index.name = 'date'
        DataType.extend("EOD")
        table_name = DataChannel.name_to_data_type(f"MIB.{symbol}", DataType.EOD)
