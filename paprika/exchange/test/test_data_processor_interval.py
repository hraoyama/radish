...
def shift_colname(column_name, num_shifts, df):
    new_column_name = f'{column_name}_L{str(num_shifts)}'
...
def test_data_processor_interval():
    ...
    data2 = DataProcessor("EUX.FDAX201709.Trade") \
        ...
        (partial(shift_colname, 'LogReturn_MEAN', 1))(partial(shift_colname, 'LogReturn_STD', 1)).data
    
    pp(np.sum(data.HIGH - data2.HIGH))
    
    plt.scatter(data2.STD, data2.LogReturn_MEAN_L1, alpha=0.5)
