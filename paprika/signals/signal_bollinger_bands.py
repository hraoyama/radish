        ...
        if len(events) == 1:
            ...
            if execute:
                ...
                regress_results = LinearRegression().fit(
                    self.prices[self.y_name][-self.look_back:-1].values.reshape(-1, 1),
                    self.prices[self.x_name][-self.look_back:-1].values.reshape(-1, 1))

                self.hedge_ratios = self.hedge_ratios.append(
                    pd.DataFrame({'DateTime': [last_index],
                                  'hedge_ratio': regress_results.coef_[0]}))

                calc_spread = -regress_results.coef_[0] * self.y_data[price_column][-1] + self.x_data[price_column][-1]

                self.spread = self.prices.append(
                    pd.DataFrame({'DateTime': [last_index],
                                  'spread': calc_spread}))
