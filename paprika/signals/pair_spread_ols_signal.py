...
class PairSpread(FeedSubscriber):
    ...
    def handle_event(self, events: List[Tuple[DataType, pd.DataFrame]]):
        ...
        if any(data.ISIN == self.get_parameter("Y")):
            self.num_y_obs = 1 if not self.num_y_obs else 0  # only keep the last observation
            self.last_y = data.loc[data.ISIN == self.get_parameter("Y"), "Price"][-1]
        if any(data.ISIN == self.get_parameter("X")):
            self.num_x_obs = 1 if not self.num_x_obs else 0
...
if __name__ == "__main__":
    ...
    # let's use the running historical standard deviation
    ...
    # let's use the running historical standard deviation and some adaptive beta
