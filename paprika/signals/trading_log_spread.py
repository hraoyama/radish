...
def main():
    ...
    for t in np.arange(lookback, n_periods + 1):
        regress_results = LinearRegression().fit(np.log(df.iloc[(t-lookback):t]['GLD'].values).reshape(-1, 1),
                                                 np.log(df.iloc[(t-lookback):t]['USO'].values))
        hedge_ratio[t-1] = regress_results.coef_[0]

    portfolio_value = np.sum(np.stack((-hedge_ratio, np.ones(n_periods)), axis=1) * np.log(df[['GLD', 'USO']]), axis=1)
    ...
    positions = pd.DataFrame(np.tile(num_units.values, [2, 1]).T * np.stack((-hedge_ratio,
